...
private Logger logger = LogManager.getLogger(this.getClass());
		...
		IServiceExecutor<?> executor = new IServiceExecutor<Object>() 
		{
			...
			public Object call() throws Exception 
			{
				...
				try 
				{
					...
					String url = prepareUrl(request,map);
					...
					org.springframework.http.ResponseEntity<String> data = restTemplate.exchange(URI.create(url),HttpMethod.GET, requestEntity, String.class);
			...
			{
				return WebClientGetService.class.getName();
			}
	...
	public String prepareUrl(HttpServletRequest request,Map<String,String> map)
	{
		String url = map.get(Constants.SERVICE_SOURCE_URL);
		logger.info("url declare-------"+url);
		String paramKey =map.get(Constants.PARAM_KEY);
		logger.info("paramKey declare-------"+paramKey);
		String pathKey = map.get(Constants.PATH_KEY);
		logger.info("pathKey declare -------"+pathKey);
		Enumeration<String> par = request.getParameterNames();
		logger.info("par-------"+par);
		
		if(pathKey.equals("true"))
		{
			while(par.hasMoreElements())
			{
				String parName = par.nextElement();
				logger.info("parName inside if while-------"+parName);
				String parValue = request.getParameter(parName);
				logger.info("parValue inside if while-------"+parValue);
				url = url+parValue;
				logger.info("url inside if while-------"+url);
			}
		}
		else if(paramKey.equals("true")) 
		{
			url = url+"?";
			logger.info("url inside else while-------"+url);
			while(par.hasMoreElements()) {
			String parName = par.nextElement();
			logger.info("parName inside else while-------"+parName);
			String parValue = request.getParameter(parName);
			logger.info("parValue inside else while-------"+parValue);
			url = url+parName+"="+parValue;
			logger.info("inside while (url+parName+\"=\"+parValue)-------"+url);
			url = url+"&";
			logger.info("inside while (url+\"&\")-------"+url);			
		}
			url = url.substring(0, url.length() - 1);
			logger.info("url outside while in else-------"+url);
	}
	return url;
  }
}
